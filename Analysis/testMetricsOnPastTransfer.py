...
top_teams_barthag = all_teams_barthag[all_teams_barthag['barthag_rank'] <= 180]
...
for idx, avail_team in sampled_teams.iterrows():
    ...
    try:
        ...
        cs_df = composite_score(conn, team_name, season_year, player_id_to_replace, roleToSort=role)                
        ...
        unsuccessPercentile = (rank >= length * bottomPercent)
        successCond = successPercentile and is_success
        unsuccessCond = (rank >= len(cs_df) * bottomPercent) and not is_success        
